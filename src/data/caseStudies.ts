...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    title: 'Football Probability & EV Engine',
    summary: 'An in-house quantitative engine that builds a full scoreline probability map for a match, prices every market from it, anchors to margin-free bookmaker consensus, and surfaces only positive expected-value selections — validated by calibration and closing-line value, not short-term wins.',
    seoTitle: 'Sports Analytics Case Study: A Football Probability & Expected-Value Engine',
    description: 'See how Naurra.ai built Total Stats EV Lab — a Poisson-based football probability engine that prices every market from one scoreline grid, anchors to market consensus, and validates with calibration and CLV.',
    heroLabel: 'Sports analytics case study',
    publishedAt: '2026-06-30',
    updatedAt: '2026-06-30',
    problem: 'Most football betting decisions are driven by hype and gut feeling, not by whether a price is actually wrong. Pricing every market — 1X2, over/under, BTTS, Asian handicaps, correct score — consistently from the same view of a match is genuinely hard, and without it a "good bet" is just a confident guess.',
    solution: 'We built a decision engine, not a tips channel. A single Poisson scoreline grid (driven by expected goals and Elo-style team strength) is computed once, and every market is derived directly from it, so the logic stays internally consistent. Bookmaker odds are stripped of margin to form a fair-probability anchor; bounded research signals can nudge the numbers but can never override the math. For each selection it outputs probability, fair odds, expected value, confidence, a CORE or LOTTERY grade, and a fractional-Kelly stake. It is built for disciplined analysis and is explicitly not financial advice or a guarantee of outcomes.',
    impact: '1 model, every market',
    impactDetail: 'One probability map prices the entire board consistently, and success is judged by process — calibration (do 60% picks win ~60%?) and closing-line value (did we beat the market price?) — rather than by individual results. Every prediction is stored so it can be audited and back-tested later.',
    challengeBullets: [
      'Selections were driven by hype instead of whether the price was genuinely wrong.',
      'Pricing 10+ markets consistently from one view of a match is hard to get right.',
      'Research narratives tend to override the math unless they are strictly bounded.'
    ],
    systemComponents: [
      'A single Poisson scoreline grid from which every market is derived',
      'Expected goals plus Elo-style team ratings to drive the grid',
      'Margin-free bookmaker consensus as the probability anchor',
    ...
    proofContext: 'Quantitative football probability & EV engine',
